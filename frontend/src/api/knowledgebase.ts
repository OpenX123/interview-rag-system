...
export interface KnowledgeBaseChunk {
  id: string;                       // vector_store.id (UUID)
  chunkIndex: number;               // 在该文档内的顺序
  totalChunks: number;              // 该文档总块数
  parentDocumentId: string | null;  // Spring AI 父文档 UUID
  content: string;                  // 切片文本
  contentLength: number;            // 切片字符数
}
...
export const knowledgeBaseApi = {
  ...
  /**
   * 获取知识库的所有向量切片（chunks），用于在管理页查看 embedding 分块结果
   */
  async getChunks(id: number): Promise<KnowledgeBaseChunk[]> {
    return request.get<KnowledgeBaseChunk[]>(`/api/knowledgebase/${id}/chunks`);
  },
