...
import ChunkViewerDrawer from '../components/ChunkViewerDrawer';
...
export default function KnowledgeBaseManagePage({ onUpload, onChat }: KnowledgeBaseManagePageProps) {
  ...
  const [chunksViewerItem, setChunksViewerItem] = useState<KnowledgeBaseItem | null>(null);
  ...
  return (
    <div className="max-w-7xl mx-auto">
                ...
                >
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-1">
                      {/* 查看分块按钮（仅 COMPLETED 状态显示，未完成的没有 chunks 可看） */}
                      {kb.vectorStatus === 'COMPLETED' && (
                        <button
                          onClick={() => setChunksViewerItem(kb)}
      ...
      {/* 向量分块查看抽屉 */}
      <ChunkViewerDrawer
        isOpen={chunksViewerItem !== null}
        knowledgeBaseId={chunksViewerItem?.id ?? null}
        knowledgeBaseName={chunksViewerItem?.name ?? ''}
        onClose={() => setChunksViewerItem(null)}
      />
    </div>
  );
}
